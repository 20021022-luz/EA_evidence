...
p_varianza <- ggplot(df_convergencia, aes(x = n, y = varianza_muestral)) +
  geom_line(color = "#8B4789", size = 1) +
  geom_hline(yintercept = varianza_teo, linetype = "dashed", color = "red") +
  labs(
    title = "Convergencia de la Varianza Muestral a la Varianza Teórica",
    subtitle = paste("Varianza Teórica =", round(varianza_teo, 4)),
    x = "Tamaño de la Muestra (n)",
    y = "Varianza Muestral"
  ) +
  theme_minimal() +
...
grid.arrange(p_media, p_varianza, ncol = 2)
# Histograma para el tamaño de muestra más grande
n_grande <- max(n_sizes)
muestra_grande <- rbeta(n_grande, a_param, b_param)
df_hist <- data.frame(x = muestra_grande)
p_hist <- ggplot(df_hist, aes(x = x)) +
  geom_histogram(aes(y = ..density..), bins = 30, fill = "#9ACD32", color = "black", alpha = 0.7) +
  stat_function(fun = dbeta, args = list(shape1 = a_param, shape2 = b_param), color = "#556B2F", size = 1) +
  labs(
    title = paste("Histograma de la Muestra (n =", n_grande, ") con PDF Teórica"),
    x = "x",
    y = "Densidad"
  ) +
  theme_minimal() +
...
for (n in n_sizes) {
  ...
  p <- ggplot(df_tcl, aes(x = medias)) +
    geom_histogram(aes(y = ..density..), bins = 30, fill = "#EE7AE9", color = "black", alpha = 0.7) +
    stat_function(fun = dnorm, args = list(mean = media_teo, sd = sd_teo), color = "#CD1076", size = 1) +
    labs(
      title = paste("TCL: n =", n),
      subtitle = paste("Media Teórica =", round(media_teo, 4),
...
grid.arrange(grobs = plots_tcl, ncol = 2,
             top = "Verificación del Teorema Central del Límite con Distribución Beta(2,5)")
