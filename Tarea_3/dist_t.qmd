...
```{r}
n_muestras1 <- 100
n <- 5 #Grados de libertad, tanto para ji-cuadrada como para t

#Generar muestras 
df_ejercicios1 <- tibble(
  Muestra = 1:n_muestras1,#Conteo muestra 
  X = rnorm(n_muestras1),#Muestra normal estandar
)

#Grafico 1
ggplot(df_ejercicios1)+
  geom_histogram(aes(x=X, y = after_stat(density)), bins = 50, fill="#B2DFEE", color="black", alpha = 0.7)+
  stat_function(fun = function(x) dt(x, df = n), color="#9ACD32", linewidth = 1.2)+
  stat_function(fun = function(x) dnorm(x), color= "#a2cd5a", linewidth = 1.2)+
  theme_bw()
```
...
#Generar muestras 
df_ejercicios2 <- tibble(
  Muestra = 1:n_muestras1,#Conteo muestra 
  X = rnorm(n_muestras1),#Muestra normal estandar
)

#Grafico 1
ggplot(df_ejercicios2)+
  geom_histogram(aes(x=X, y = after_stat(density)), bins = 50, fill="#B2DFEE", color="black", alpha = 0.7)+
  stat_function(fun = function(x) dt(x, df = n), color="#8B3A62", linewidth = 1.2)+
  stat_function(fun = function(x) dnorm(x), color= "#CD1076", linewidth = 1.2)+
  theme_bw()
```
...
ggplot(df_ejercicios1)+
  geom_histogram(aes(x=X, y = after_stat(density)), bins = 50, fill="#B2DFEE", color="black", alpha = 0.7)+
  stat_function(fun = function(x) dt(x, df = n), color="#FFC0C8", linewidth = 1.2)+
  stat_function(fun = function(x) dnorm(x), color= "#EE7AE9", linewidth = 1.2)+
  theme_bw()
```
Mostrar la varianza y media muestral 

```{r}
media_1 <- function(n_muestras1){
  muestra <- rnorm(n_muestras1)
  media <- mean(muestra)
  sd <- sd(muestra)
  return(c(media, sd))
}

valores_ej1 <- replicate(100, media_1(n_muestras1))
```
```{r}
media_2 <- function(n_muestras2){
  muestra2 <- rnorm(n_muestras2)
  media <- mean(muestra2)
  sd <- sd(muestra2)
  return(c(media, sd))
}

valores_ej2 <- replicate(300, media_2(n_muestras2))
```
```{r}
media_3 <- function(n_muestras3){
  muestra3 <- rnorm(n_muestras3)
  media <- mean(muestra3)
  sd <- sd(muestra3)
  return(c(media, sd))
}

valores_ej3 <- replicate(1000, media_3(n_muestras3))
```
...
```{r}
n_muestras <- 5000
n <- 10 #Grados de libertad, tanto para ji-cuadrada como para t

#Generar muestras 
df_ejercicios <- tibble(
  Muestra = 1:n_muestras,#Conteo muestra 
  X = rnorm(n_muestras),#Muestra normal estandar
  Ji = rchisq(n_muestras, df = n),#Muestra Ji-cuadrada 
  T = X/sqrt(Ji/n)#calcular T 
)

#Histograma de T muestra con densidad teorica t(n)
ggplot(df_ejercicios)+
  geom_histogram(aes(x=T, y = after_stat(density)), bins = 50, fill="#FFC0C8", color="black", alpha = 0.7)+
  stat_function(fun = function(x) dt(x, df = n), color="#CD1076", linewidth = 1.2)+
  stat_function(fun = function(x) dnorm(x), color= "#B2DFEE", linewidth = 1.2)+
  theme_bw()
```
...
ggplot(df_exr2)+
  geom_histogram(aes(x=T, y = after_stat(density)), bins = 100, fill="lightpink", color = "black", alpha = 0.7)+
  geom_function(fun = function(x) dt(x, df=n_mue-1), color ="#68838B", linewidth=1.2)+
  theme_bw()
```
