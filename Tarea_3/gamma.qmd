...
```{r}
#| label: cuantiles_gamma1

# Mostrar cuantiles en el gráfico de CDF
alpha_q1 <- 2.5
lambda_q1 <- 1.8
x_q <- seq(0, 15, length.out = 1000)
cdf_q <- pgamma(x_q, shape = alpha_q1, rate = lambda_q1)

# Cuantiles específicos
p_vals1 <- c(5, 10, 20, 30, 50, 100)
q_vals1 <- qgamma(p_vals1, shape = alpha_q1, rate = lambda_q1)

# Crear puntos para los cuantiles
df_cuantiles <- data.frame(
  x = q_vals1,
  y = p_vals1,
  etiqueta = paste0("Q", p_vals1*100, " = ", round(q_vals1, 2))
)
...
```{r}
# Visualizar el efecto de los parámetros en la media
x_eje1 <- seq(0, 15, length.out = 1000)

df_alpha_media1 <- data.frame()
for (a in alpha_q1) {
  media_a1 <- alpha_q1 / lambda_q1
  temp1 <- data.frame(
    x = x_eje1,
    y = dgamma(x_eje1, shape = alpha_q1, rate = lambda_q1),
    parametro = paste("α =", alpha_q1),
    media = media_a1,
    tipo = "Muestra de  α & λ (Media) "
  )
  df_alpha_media1 <- rbind(df_alpha_media1, temp1)
}

df_lambda_media1 <- data.frame()
for (b in lambda_q1) {
  media_b1 <- lambda_q1 / alpha_q1
  temp2 <- data.frame(
    x =x_eje1,
    y = dgamma(x_eje1, shape = alpha_q1, rate = lambda_q1),
    parametro = paste("λ =", lambda_q1),
    media = media_b1,
    tipo = "Comparación  α & λ (Varianza)"
  )
  df_lambda_media1 <- rbind(df_lambda_media1, temp2)
}

# Combinar datos
df_medias1 <- rbind(df_alpha_media1, df_lambda_media1)

# Crear gráfico
ggplot(df_medias1, aes(x = x, y = y, color = parametro)) +
  geom_line(linewidth = 1.2) +
  geom_vline(aes(xintercept = media, color = parametro), 
             linetype = "dashed", alpha = 0.7) +
  facet_wrap(~tipo, scales = "free", ncol = 2) +
  labs(title = " Media & Varianza de la Distribución Gamma",
       subtitle = "Las líneas verticales indican las medias teóricas",
       x = "x", y = "f(x)",
       color = "Parámetro") +
  scale_color_brewer(palette="Dark2")+
...
```{r tcl-setup}
# Parámetros es la distribución gamma
alpha_tcl <- 2.5
lambda_tcl <- 1.8
media_poblacional <- alpha_tcl/lambda_tcl
varianza_poblacional <- alpha_tcl/lambda_tcl^2
...
generar_medias <- function(n, num_sim,alpha, lambda) {
  medias <- replicate(num_sim, mean(rgamma(n,alpha, rate = lambda)))
  return(medias)
}
...
for (i in seq_along(size_muestra)) {
  n <- size_muestra[i]
  medias <- generar_medias(n, num_muestras, alpha_tcl, lambda_tcl)
...
df_normal_teorica$n_factor <- factor(df_normal_teorica$n_factor, 
                         levels = paste("n =", size_muestra))
...
ggplot(df_tcl, aes(x = medias)) +
  ...
  facet_wrap(~n_factor, ncol = 3) +
  labs(title = "Teorema Central del Límite - Distribución Gamma",
       subtitle = paste("Distribución de medias muestrales para Gamma (alpha =", alpha_tcl, "lambda =", lambda_tcl, ")"),
       x = "Media muestral",
       y = "Densidad",
