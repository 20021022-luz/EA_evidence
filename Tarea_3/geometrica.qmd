...
```
    ```{r}
# Calcular probabilidades para los primeros 5 ensayos
k_moneda_5 <- 0:6
prob_geometrica_e <- dgeom(k_moneda_5, p_cara)  

# Crear tabla
geometrica_moneda_e<- data.frame(
  Ensayo = k_moneda_5,
  Probabilidad = prob_geometrica_e,
  Prob_Acumulada = pgeom(k_moneda_5, p_caras)
)

kable(geometrica_moneda_e, 
      caption = "Distribución Geométrica - Primera cara (p = 0.5)",
      digits = 4,
      col.names = c("Ensayo", "P(X = x)", "P(X ≤ x)")) %>%
  kable_styling(bootstrap_options = "striped")
...
# Parámetros estadísticos
media_moneda <- (1-p_caras) / p_caras
varianza_moneda <- (1 - p_caras) / (p_caras^2)
desv_std_moneda <- sqrt(varianza_moneda)
...
# Calcular probabilidades para los primeros 5 ensayos
k_moneda_3 <- 0:4
prob_geometrica_e <- dgeom(k_moneda_3, p_caras)  

# Crear tabla
geometrica_moneda_e<- data.frame(
  Ensayo = k_moneda_3,
  Probabilidad = prob_geometrica_e,
  Prob_Acumulada = pgeom(k_moneda_3, p_caras)
)
...
```
```{r}
#Primera tabla con 5 lanzamientos
#| label: grafico-geometrica-basico
#| fig-cap: "Distribución Geométrica - Primera cara en lanzamiento de moneda"
ggplot(geometrica_moneda_e, aes(x = Ensayo, y = Probabilidad)) +
  geom_col(fill = "#EE7AE9", alpha = 0.7, width = 0.8) +
  geom_text(aes(label = round(Probabilidad, 3)), vjust = -0.5, size = 3) +
  geom_vline(xintercept = media_moneda, color = "#9ACD32", linetype = "dashed", linewidth = 1) +
  annotate("text", x = media_moneda + 1.5, y = max(prob_geometrica) * 0.8, 
           label = paste("E[X] =", media_moneda), color = "#9ACD32") +
  scale_x_continuous(breaks = k_moneda_3) +
  labs(title = paste("Distribución Geométrica(", p_caras, ")"),
       subtitle = "Número de lanzamientos antes de obtener la primera cara",
       x = "Número de Ensayos",
       y = "Probabilidad") +
  theme_minimal() +
...
ggplot(jugador)+
  geom_col(aes(Tiros, Probabilidad), fill="#8B3A62", alpha = 0.2, color="black")
  theme_bw()

esperanza <- (1-p)/p

cat("b) El numero de intentos que se espera que necesite anotar:", esperanza +1)
```
:::
...
#| label: control-calidad-geometrica
# Parámetros
p_defecto_geom_e <- 0.02

# Parámetros estadísticos
media_defecto <- (1-p_defecto_geom_e) / p_defecto_geom_e
varianza_defecto <- (1 - p_defecto_geom_e) / (p_defecto_geom_e^2)
desv_std_defecto <- sqrt(varianza_defecto)

cat("Control de Calidad - Primera pieza defectuosa:\n")
cat("Probabilidad de defecto por pieza:", p_defecto_geom_e, "\n")
cat("Número esperado de inspecciones:", media_defecto, "\n")
cat("Desviación estándar:", round(desv_std_defecto, 2), "\n\n")

# Probabilidades específicas
k_calidad_e <- 0:300
prob_calidad <- dgeom(k_calidad_e, p_defecto_geom_e)

# Preguntas de interés
prob_primera_100 <- sum(prob_calidad[1:100])
prob_mas_200 <- 1 - sum(prob_calidad[1:200])
prob_entre_100_300 <- sum(prob_calidad[100:300])

cat("Probabilidades de interés:\n")
cat("P(primera defectuosa en ≤ 100 inspecciones):", round(prob_primera_100, 4), "\n")
cat("P(primera defectuosa en > 200 inspecciones):", round(prob_mas_200, 4), "\n")
cat("P(primera defectuosa entre 100 y 300 inspecciones):", round(prob_entre_100_300, 4), "\n")



calidad_data <- data.frame(
  k = k_calidad_e[1:300],
  probabilidad = prob_calidad[1:300]
)


ggplot(calidad_data, aes(x = k, y = probabilidad)) +
  geom_col(fill = "#CD1076", alpha = 0.7, width = 0.8) +
  geom_vline(xintercept = media_defecto, color = "#FFC0C8", linetype = "dashed", size = 1) +
  annotate("text", x = media_defecto + 5, y = max(prob_calidad) * 0.8, 
           label = paste("E(X) =", media_defecto), color = "#FFC0C8") +
  labs(title = "Control de Calidad: Primera Pieza Defectuosa",
       subtitle = paste("Geométrica(", p_defecto_geom_e, ")"),
       x = "Número de Inspecciones",
       y = "Probabilidad") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
