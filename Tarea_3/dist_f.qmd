...
df_ejer_c <- tibble(
  Muestra = 1:n_mues,
  Ji_a = rchisq(n_mues, df = a),
  Ji_b = rchisq(n_mues, df = b),
  F_sim = (Ji_a/a)/(Ji_b/b)
)

#Histograma
ggplot(df_ejer_c)+
  geom_histogram(aes(x = F_sim, y = after_stat(density)), bins = 100, fill="#68838B", color="black", alpha=0.7)+
  geom_function(fun=function(x) df(x, a, b), color ="#8B3A62", linewidth = 1.2)+
  xlim(0, 7.5)+
  theme_bw()
```
