...
dis_altura <- tibble(
  ...
  cdf_altura = pnorm(estaturas, mean=mu_altura, sd = sigma_altura)
)
  ggplot(dis_altura) +
  geom_line(aes(x = estaturas, y = pdf_altura), color = "pink", linewidth = 1.3) +
  geom_area(data = dis_altura |> filter(estaturas >= 180), aes(estaturas, pdf_altura), fill = "#8B4789", alpha = 0.5) +
  geom_vline(xintercept = 180, linetype = "dashed") +
  annotate("text", x = 190, y = 0.01, label = paste("1-F(180):", pnorm(180, mu_altura, sigma_altura, lower.tail = FALSE))) +
  theme_bw()
...
```{r}
ggplot()+
  geom_histogram(aes(muestra_altura, y=after_stat(density)), fill="#FFC0C8", color = "#EE7AE9", center = 170)+
  geom_line(data = dis_altura, aes(estaturas, pdf_altura), color = "#9AC0CD", linewidth = 1.3)+
  theme_bw()
...
#Generamos la muestra
mu_ventas <- 50000
sigma_ventas <- 12000^2
muestra_ventas <- rnorm(24, mean = mu_ventas, sd= sigma_ventas)
media_ventas <- mean(muestra_ventas)
mediana_ventas <- median(muestra_ventas)
sd_ventas <- sd(muestra_ventas)

cat("La media de las ventas:", media_ventas, "\n")
cat("La mediana de la altura es:", mediana_altura, "\n")
cat("La sd de la altura es:", sd_altura, "\n")
