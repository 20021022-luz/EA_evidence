...
#| code-fold: true
#| label: simulacion
# Función para generar números de distribución uniforme discreta
runif_disc <- function(n, a, b) {
...
Verifiquemos el Teorema Central del Límite mediante simulación para el caso de variables aleatorias con distribución uniforme discreta. Tomaremos 1000 muestras de tamaño 30 de una distribución uniforme discreta en el conjunto $\{1, 2, 3, 4, 5, 6\}$ y observaremos la distribución de sus medias muestrales.
...
# Mostrar tabla
kable(dado_data, 
      caption = "Distribución de probabilidad de una bolsa con 8 pelotas numeradas",
      align = "cc") %>%
  kable_styling(bootstrap_options = "striped")
```
...
#| code-fold: true
#| label: grafico-bolsa
#| fig-cap: "Función de probabilidad de sacar una pelota de una bolsa"

ggplot(dado_data, aes(x = Valor, y = Probabilidad)) +
  geom_col(fill = "#8B3A62", alpha = 0.7, width = 0.6) +
  geom_text(aes(label = round(Probabilidad, 3)), vjust = -0.5) +
  scale_x_continuous(breaks = valores) +
  scale_y_continuous(limits = c(0, 0.20)) +
  labs(title = "Distribución Uniforme Discreta - bolsa ",
       x = "Valor",
       y = "Probabilidad") +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5),
...
x2_1e <- 5
x2_ne <- 15
valores4 <- x2_1e:x2_ne
probabilidades4 <- rep(1/(length(valores4)), length(valores4))

# Data_frame
data4 <- data.frame(
  Valor = valores2,
  Probabilidad = probabilidades4
)

# Parámetros estadísticos
media4 <- mean(data2$Valor)
varianza4 <- sum((data2$Valor - media4)^2)/length(data4$Valor)
desv_std4 <- sqrt(varianza2)

cat("Media:", media4, "\n")
cat("Varianza:", round(varianza4, 4), "\n")
cat("Desviación estándar:", round(desv_std4, 4), "\n")
```
**Visualización de la función de probabilidad:**
...
ggplot(data4, aes(x = Valor, y = Probabilidad)) +
  geom_col(fill = "#8B4789", alpha = 0.7, width = 0.8) +
  geom_text(aes(label = round(Probabilidad, 3)), 
            vjust = -0.5, size = 3) +
...
:::{#Solución ejercicio simulación_4}
...
ggplot(comp_long, aes(x = Valor, y = Probabilidad, fill = Tipo)) +
  ...
  scale_fill_manual(values = c("Teórica" = "#FFC0C8", "Observada" = "#CD1076")) +
  labs(title = paste("Simulación con", n_sim, "observaciones"),
       x = "Valor",
       y = "Probabilidad",
       fill = "Distribución") +
