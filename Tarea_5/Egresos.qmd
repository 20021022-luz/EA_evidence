...
El examen de los **gastos realizados** por los estados es fundamental para determinar cómo se están distribuyendo los fondos públicos, qué *prioridades* se están estableciendo en las políticas de gobierno y cuál es la solidez financiera de las administraciones estatales para pagar por servicios esenciales y obras de infraestructura.

***¿Por qué es importante analizar el Gasto Estatal?***

Analizar a fondo la composición del desembolso estatal nos permite identificar varios aspectos cruciales:

-   **Desequilibrios y Disparidades:** Revela si existen distribuciones inadecuadas o desproporcionadas en el gasto entre diferentes áreas.

-   **Rigideces Presupuestarias**: Ayuda a detectar compromisos de gasto que son difíciles de modificar (como nóminas o deuda), limitando la flexibilidad para invertir en nuevas iniciativas.

-   **Oportunidades de Eficiencia**: Señala dónde se puede mejorar la efectividad y el óptimo aprovechamiento de los recursos disponibles, asegurando que cada peso gastado genere el mayor beneficio para la ciudadanía.

En esencia, al comprender la estructura de los egresos, podemos evaluar la calidad del gasto y proponer medidas para hacerlo más eficiente, equitativo y alineado con las necesidades de la población.

## Estructura

```{r}
# Crear el data frame con la estructura del gasto
estructura_gasto <- data.frame(
  Categoria_Principal = c(
    "Gasto Corriente", "Gasto Corriente", "Gasto Corriente", "Gasto Corriente",
    "Gasto de Capital", "Gasto de Capital", "Gasto de Capital",
    "Resto de Egresos", "Resto de Egresos", "Resto de Egresos"
  ),
  Concepto = c(
    "Sueldos y salarios",
    ...
    "Participaciones y otros pasivos",
    "Variaciones en disponibles"
  )
)

# Imprimir el data frame para verificar
print(estructura_gasto)
```
...
## Vista Rápida a la Estructura de Egresos por Capítulo de Gasto.

### Estructura General de los Datos

-   **`Egresos`**: Filtrado para el tema "Egresos" y categoría "Capítulo". Se agrega porcentaje anual de cada capítulo.
-   **`Egresos_agregados`**: Agregado por año, estado y categoría principal con cálculos de porcentaje anual.
-   **`Egresos_wide`**: Transformado a formato ancho con valores y porcentajes por categoría principal.
...
egresos_agregados <- egresos |> 
  ...
  ungroup() |> 
  dplyr::select(ANIO, CVE_ENT, VALOR, ANIO_factor, VALOR_millones, porcentaje_anual, NOM_ENT, ABR_ENT, ZONA, CATEGORIA_PRINCIPAL)
...
print(mapa)
```
  Podemos notar una gran diferencia de resultados (una mayor variabilidad), dejando de lado a la CDMX. Objeto que sirve en su generalidad para este analisis.
...
ggplot(egresos_agregados |> filter(ABR_ENT =="ZAC"), aes(x = ANIO_factor, y =porcentaje_anual,
       fill = CATEGORIA_PRINCIPAL)) +
  geom_col(position = "stack") +
  geom_text(aes(label = ifelse(porcentaje_anual >= 5, paste0(round(porcentaje_anual, 1), "%"), "")), 
            position = position_stack(vjust = 0.5),
  ...
  scale_fill_viridis_d(option = "turbo") +
  labs(
    title = "Composición de Egresos de Zacatecas (2013-2023)",
    subtitle = "Porcentaje por categoría (etiquetas >5%)",
    x = "Año",
    y = "Porcentaje (%)",
    fill = "Categoría"
...
ggplot(egresos_agregados |> filter(ABR_ENT =="VER"), aes(x = ANIO_factor, y =porcentaje_anual,
  ...
  scale_fill_viridis_d(option = "turbo") +
  labs(
    title = "Composición de Egresos de Veracruz de Ignacio de la Llave (2013-2023)",
    subtitle = "Porcentaje por categoría (etiquetas >5%)",
    x = "Año",
    y = "Porcentaje (%)",
    fill = "Categoría"
...
ggplot(egresos_agregados |> filter(ABR_ENT =="CHIS"), aes(x = ANIO_factor, y =porcentaje_anual,
       fill = CATEGORIA_PRINCIPAL)) +
  geom_col(position = "stack") +
  geom_text(aes(label = ifelse(porcentaje_anual >= 5, paste0(round(porcentaje_anual, 1), "%"), "")), 
            position = position_stack(vjust = 0.5),
  ...
  scale_fill_viridis_d(option = "turbo") +
  labs(
    title = "Composición de Egresos de Chiapas (2013-2023)",
    subtitle = "Porcentaje por categoría (etiquetas >5%)",
    x = "Año",
    y = "Porcentaje (%)",
    fill = "Categoría"
...
Se realizará un examen pormenorizado del *gasto de capital*, destacándolo como un elemento crucial de los egresos gubernamentales en las entidades federativas mexicanas entre 2013 y 2023. Para comenzar, se ilustrará la **distribución anual** de los porcentajes de esta categoría en cada estado mediante diagramas de caja.
...
Se lleva a cabo un Análisis de Varianza (ANOVA) con el fin de evaluar la existencia de disparidades significativas en el promedio de los porcentajes anuales correspondientes a esta categoría.
