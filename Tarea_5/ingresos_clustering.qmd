...
Se seleccionan las variables numéricas y de un año en particular, en este caso 2017.
...
ingresos_year_bar <- ingresos_agregados |> 
  filter(ANIO == my_year) 

ingresos_year <- ingresos_wide |> 
  filter(ANIO == my_year)


ingresos_wo_MEX <- ingresos_year |> filter(ABR_ENT != "MEX")
...
row.names(ingresos_numeric_valor) <- ingresos_wo_MEX$ABR_ENT

ingresos_numeric_porcentaje <- ingresos_year |> 
  dplyr::select(
    Propios_porcentaje,
...
row.names(ingresos_numeric_porcentaje) <- ingresos_year$ABR_ENT
...
dist.eucl <- dist(ingresos_numeric_porcentaje)
...
rownames(ingresos_numeric_porcentaje)[grp_euc_complete == 3]
```
...
fviz_cluster(list(data= ingresos_numeric_porcentaje, cluster= grp_euc_complete),
...
grp_single_df <- grp_single_df |> 
  rownames_to_column(var = "ABR_ENT")
fviz_cluster(list(data= ingresos_numeric_porcentaje, cluster= grp_euc_single),
             palette = "jco", ellipse.type = "convex", repel = TRUE, show.clust.cent = FALSE, ggtheme = theme_bw())
```
...
grp_ave_df <- grp_ave_df |> 
  rownames_to_column(var = "ABR_ENT")
fviz_cluster(list(data= ingresos_numeric_porcentaje, cluster= grp_euc_ave),
             palette = "jco", ellipse.type = "convex", repel = TRUE, show.clust.cent = FALSE, ggtheme = theme_bw())
```
...
grp_ward2_df <- grp_ward2_df |> 
  rownames_to_column(var = "ABR_ENT")
fviz_cluster(list(data= ingresos_numeric_porcentaje, cluster= grp_euc_ward2),
             palette = "jco", ellipse.type = "convex", repel = TRUE, show.clust.cent = FALSE, ggtheme = theme_bw())
```
...
km3 <- kmeans(ingresos_numeric_porcentaje, 3, nstart = 25)
km3_df <- as.data.frame(km3$cluster)
km3_df <- km3_df |> 
  rownames_to_column(var = "ABR_ENT")
colnames(km3_df)[2] <- "km3_cluster"
km3_df$km3_cluster <- as.factor(km3_df$km3_cluster)
fviz_cluster(km3, data = ingresos_numeric_porcentaje, palette= "jco", ellipse.type = "euclid", star.plot=TRUE, ggtheme=theme_bw())
```
...
km4 <- kmeans(ingresos_numeric_porcentaje, 4, nstart = 25)
km4_df <- as.data.frame(km4$cluster)
km4_df <- km4_df |> 
  rownames_to_column(var = "ABR_ENT")
colnames(km4_df)[2] <- "km4_cluster"
km4_df$km4_cluster <- as.factor(km4_df$km4_cluster)
fviz_cluster(km4, data = ingresos_numeric_porcentaje, palette= "jco", ellipse.type = "euclid", star.plot=TRUE, ggtheme=theme_bw())
```
...
dist.eucl <- dist(ingresos_numeric_porcentaje)
dist.eucl_matrix <- as.matrix(dist.eucl)
dist.eucl_plot_02 <- fviz_dist(dist.eucl, lab_size = 6)+
  theme(axis.text.x = element_text(size=15),
        axis.text.y = element_text(size=15))

ggplotly(dist.eucl_plot_02)
...
euc_comp_hc_02 <- hclust(dist.eucl, method = "complete")
fviz_dend(euc_comp_hc, k=5, cex=0.9, k_colors = "jco", rect=TRUE, rect_border= "jco", rect_fill = TRUE, labels_track_height=3)
...
euc_single_hc_02 <- hclust(dist.eucl, method = "single")
fviz_dend(euc_single_hc, k=5, cex=0.5, k_colors = "jco", rect=TRUE, rect_border= "jco", rect_fill = TRUE, labels_track_height=0.1)
```
...
grp_euc_single_02 <- cutree(euc_single_hc, k=5)
grp_single_df_02 <- as.data.frame(grp_euc_single_02)
grp_single_df_02 <- grp_single_df_02 |> 
  rownames_to_column(var = "ABR_ENT")
fviz_cluster(list(data= ingresos_numeric_porcentaje, cluster= grp_euc_single),
             palette = "jco", ellipse.type = "convex", repel = TRUE, show.clust.cent = FALSE, ggtheme = theme_bw())
```
...
nacional_estado_02 <- left_join(nacional_estado, grp_single_df , by = "ABR_ENT")
nacional_estado$grp_euc_single <- as.factor(nacional_estado$grp_euc_single)
# nacional_estado <- nacional_estado |>  mutate(grp_euc_complete = if_else(is.na(grp_euc_complete), "Otro", grp_euc_complete))

mapa_02 <- ggplot(nacional_estado) +
  geom_sf(aes(fill=grp_euc_single), color = "gray50", size = 0.5) +
  labs(
    title = "Clustering, linkage single ",
    fill = "Cluster"
...
euc_ave_hc_02 <- hclust(dist.eucl, method = "average")
fviz_dend(euc_ave_hc, k=5, cex=0.5, k_colors = "jco", rect=TRUE, rect_border= "jco", rect_fill = TRUE, labels_track_height=0.25)
```
...
grp_euc_ave_03 <- cutree(euc_ave_hc, k=5)
grp_ave_df_03 <- as.data.frame(grp_euc_ave_03)
grp_ave_df_03 <- grp_ave_df_03 |> 
  rownames_to_column(var = "ABR_ENT")
fviz_cluster(list(data= ingresos_numeric_porcentaje, cluster= grp_euc_ave),
             palette = "jco", ellipse.type = "convex", repel = TRUE, show.clust.cent = FALSE, ggtheme = theme_bw())
```
...
nacional_estad0_03 <- left_join(nacional_estado, grp_ave_df , by = "ABR_ENT")
nacional_estado$grp_euc_ave <- as.factor(nacional_estado$grp_euc_ave)
# nacional_estado <- nacional_estado |>  mutate(grp_euc_complete = if_else(is.na(grp_euc_complete), "Otro", grp_euc_complete))

mapa_03 <- ggplot(nacional_estado) +
  geom_sf(aes(fill=grp_euc_ave), color = "gray50", size = 0.5) +
  labs(
    title = "Clustering, linkage average",
    fill = "Cluster"
...
euc_ward2_hc_03 <- hclust(dist.eucl, method = "ward.D2")
fviz_dend(euc_ward2_hc_03, k=5, cex=0.5, k_colors = "jco", rect=TRUE, rect_border= "jco", rect_fill = TRUE, labels_track_height=0.25)
```
...
grp_euc_ward2_02 <- cutree(euc_ward2_hc_03, k=5)
grp_ward2_df_04 <- as.data.frame(grp_euc_ward2_02)
grp_ward2_df_04 <- grp_ward2_df_04 |> 
  rownames_to_column(var = "ABR_ENT")
fviz_cluster(list(data= ingresos_numeric_porcentaje, cluster= grp_euc_ward2),
             palette = "jco", ellipse.type = "convex", repel = TRUE, show.clust.cent = FALSE, ggtheme = theme_bw())
```
...
nacional_estado_03 <- left_join(nacional_estado_02, grp_ward2_df_04, by = "ABR_ENT")
nacional_estado$grp_euc_ward2 <- as.factor(nacional_estado$grp_euc_ward2)
# nacional_estado <- nacional_estado |>  mutate(grp_euc_complete = if_else(is.na(grp_euc_complete), "Otro", grp_euc_complete))

mapa_04 <- ggplot(nacional_estado) +
  geom_sf(aes(fill=grp_euc_ward2), color = "gray50", size = 0.5) +
  labs(
    title = "Clustering, linkage Ward",
    fill = "Cluster"
...
km3_02 <- kmeans(ingresos_numeric_porcentaje, 3, nstart = 25)
km3_df_02 <- as.data.frame(km3_02$cluster)
km3_df_02 <- km3_df_02 |> 
  rownames_to_column(var = "ABR_ENT")
colnames(km3_df_02)[2] <- "km3_cluster"
km3_df_02$km3_cluster <- as.factor(km3_df_02$km3_cluster)
fviz_cluster(km3_02, data = ingresos_numeric_porcentaje, palette= "jco", ellipse.type = "euclid", star.plot=TRUE, ggtheme=theme_bw())
```
...
nacional_estado_04 <- left_join(nacional_estado, km3_df , by = "ABR_ENT")

# nacional_estado <- nacional_estado |>  mutate(grp_euc_complete = if_else(is.na(grp_euc_complete), "Otro", grp_euc_complete))

mapa_05 <- ggplot(nacional_estado_04) +
  geom_sf(aes(fill= km3_cluster), color = "gray50", size = 0.5) +
  labs(
    title = "K means, K=3",
    fill = "Cluster"
