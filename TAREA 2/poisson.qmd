...
prob_0 <- dpois(0, lambda_30seg)

# c) Llamadas esperada por hora
llamadas_esperadas <- lambda * 60

# d) Probabilidad entre 8 y 12 llamadas en dos minutos
lambda_2min <- lambda*2
prob_entre_8_12 <- sum(dpois(8:12, lambda_2min))

cat("a) La probabilidad de recibir exactamente 4 llamadas en un minuto es", 
    round(prob_4, 6), "\n")

cat("b) La probabilidad de no recibir ninguna llamada en 30 segundos",
    round(prob_0, 6), "\n")

cat("c) Las llamadas esperadas por hora es", 
    round(llamadas_esperadas , 6), "\n")

cat("d) La probabilidad de recibir entre 8 y 12 llamadas (inclusive) en dos minutos es",
    round(prob_entre_8_12,6), "\n")
```
...
# b) P(X mas de 10)
lambda_500 <- lambda*5
prob_mas_10 <- 1- ppois(10, lambda_500)

# c) Productos para que P(X)=0.95
lambda_prom <- 1.5/100
prob_al_menos_un_defecto <- 0.95
x <- -log(1 - prob_al_menos_un_defecto) / lambda_prom

# d)  Nueva P(X mas de 10)
lambda_nueva <- 1.2*5
prob_mas_10_2 <- 1- ppois(10, lambda_nueva)

cat("a) La probabilidad de tener exactamente 8 defectos en 500 piezas:", 
    round(prob_8, 4), "\n")

cat("b) La probabilidad de tener 10 o más defectos en 500 producto:",
    round(prob_mas_10, 4), "\n")

cat("c) Productos se deben fabricar para que la probabilidad de tener al menos un defecto sea de 0.95:", 
    round(x), "\n")

cat("d) La probabilidad de tener 10 o más defectos en 500 productos con una nueva tasa de 1.2:",
    round(prob_mas_10_2, 4), "\n")
...
# b) P(X = 0) en 2 hrs
lambda_2_hrs <- lambda*2
prob_0 <- dpois(0, lambda_2_hrs)

# c) P para superar la capacidad
lambda_4 <- lambda*4
prob_sup_capacidad <- 1- ppois(8, lambda_4)

# d)  Capacidad minima
capacidad_minima <- qpois(0.90, lambda_4)

cat("a) La probabilidad de que lleguen exactamente 3 pacientes en una hora:", 
    round(prob_3, 4), "\n")

cat("b) La probabilidad de que no llegue ningún paciente en 2 horas consecutivas:",
    round(prob_0, 4), "\n")

cat("c) La probabilidad de superar esta capacidad de atender 8 pacientes por turno de 4 horas:", 
    round(prob_sup_capacidad, 4), "\n")

cat("d) Capacidad mínima para que la probabilidad de saturación sea menor al 10% en un turno de 4 horas:",
    round(capacidad_minima, 4), "\n")
...
# Calcular errores
estadisticas_tcl$Error_Media <- abs(estadisticas_tcl$Media_Observada - estadisticas_tcl$Media_Teorica)
estadisticas_tcl$Error_Var <- abs(estadisticas_tcl$Var_Observada - estadisticas_tcl$Var_Teorica)
estadisticas_tcl$Error_SD <- abs(estadisticas_tcl$SD_Observada - estadisticas_tcl$SD_Teorica)
...
# Histogramas de las medias muestrales
ggplot(data_tcl, aes(x = media_muestral)) +
  geom_histogram(aes(y = after_stat(density)), bins = 30, alpha = 0.7, fill = "#CD1076") +
  geom_vline(xintercept = lambda_tcl, color = "#556B2F", linetype = "dashed", linewidth = 1) +
  facet_wrap(~n_label, scales = "free_y") +
  labs(title = "Distribución de Medias Muestrales (TCL)",
       subtitle = paste("Distribución original: Poisson(λ =", lambda_tcl, "). Línea roja: media teórica"),
       x = "Media muestral",
...
plots_comparacion <- lapply(names(data_tcl_split), function(label) {
  ...
  ggplot(datos, aes(x = media_muestral)) +
    geom_histogram(aes(y = after_stat(density)), bins = 25, alpha = 0.6, fill = "#CD1076") +
    stat_function(fun = dnorm, 
                  args = list(mean = lambda_tcl, sd = sqrt(lambda_tcl/n_val)), 
                  color = "#556B2F", size = 1.2) +
    ggtitle(label) +
    xlab("Media muestral") +
    ylab("Densidad") +
    theme_minimal()
...
data_suma_comp <- data.frame(
  valores = c(suma_simulada, poisson_directa),
  metodo = rep(c("X₁ + X₂", "Poisson(λ₁+λ₂)"), each = n_sim_suma)
)
...
ggplot(data_suma_comp, aes(x = valores, fill = metodo)) +
  ...
  theme_minimal() +
  theme(legend.position = "top") +
  scale_fill_manual(values = c("X₁ + X₂" = "darkorange1", "Poisson(λ₁+λ₂)" = "darkolivegreen"))
```
```{r}
# Suma de 4 variables Poisson independientes
lambdas <- c(7.1, 9.3, 2.7, 5.4)
lambda_total <- sum(lambdas)
n_vars <- length(lambdas)
...
ggplot(data_multiple, aes(x = valores, fill = tipo)) +
  ...
  labs(title = "Suma de Múltiples Variables Poisson",
       ...
       fill = "Tipo") +
  theme_minimal() +
  scale_fill_manual(values = c("#CD1076", "#8B4789"))
```
:::
...
# a) P(X >25)
prob_mas_25 <- 1- ppois(25, lambda)

# b) P(X >50)
prob_mas_50 <- 1- ppois(50, lambda)

# c) Capacidad óptima
capacidad_optima <- qpois(0.99, lambda)

# d)  Porcentaje degradación
set.seed(123)
simulacion <- rpois(60, lambda)
minutos_degradacion <- sum(simulacion > capacidad_optima)
porcentaje_degradacion <- minutos_degradacion / 60 * 100


cat("a) La probabilidad de que en un minuto el servidor experimente degradación:", 
    round(prob_mas_25, 4), "\n")

cat("b) Si se duplica la capacidad del servidor (50 solicitudes), ¿cómo cambia la probabilidad de degradación:",
    prob_mas_50, "\n")

cat("c) Capacidad óptima del servidor para que la probabilidad de degradación sea menor al 1%:", 
    round(capacidad_optima), "\n")

cat("d)  Simule el comportamiento del servidor durante una hora (60 minutos) y calcule el porcentaje de tiempo con degradación:",
    porcentaje_degradacion, "\n")
...
# a) P(X > 12)
prob_mas_12 <- 1- ppois(12, lambda)

# b) Unidades probabilidad de agotarse menor al 5%
unidades <- qpois(0.95, lambda)

# c) Costo esperado diario


# d)  Nivel óptimo de inventario que minimiza el costo total esperado



cat("a) La probabilidad de quedarse sin existencias si tienen un inventario de 12 piezas:", 
    round(prob_mas_12, 4), "\n")

cat("b) Unidades que debe tener para que la probabilidad de agotarse sea menor al 5%:",
   round(unidades), "\n")

cat("c) Costo esperado diario:", 
    round(capacidad_optima), "\n")

cat("d)  Nivel óptimo de inventario que minimiza el costo total esperado:",
    porcentaje_degradacion, "\n")
```
