...
geometrica_binneg <- function(r = 8, p = 0.5, n_sim) { 
  
  geometrica_sim <- replicate(n_sim, sum(rgeom(r,prob = p)))
  binneg_sim <- rnbinom(n_sim, size = r, prob = p)
  
  data_hist <- data.frame(
    valores = c(geometrica_sim, binneg_sim),
    distribucion = rep(c("Geométrica", "Binomial Negativa"),
                       each = n_sim)
  )
...
ggplot(data_hist, aes(x=valores, fill=distribucion))+
  geom_histogram(alpha = 0.7, bins = 20, position = "identity")+
  facet_wrap(~distribucion, scales = "free")+
  labs(tittle = "Histogramas de las simulaciones", subtitle = paste ("p=", p, ", n=",
  format(n_sim, big.mark = ",")), 
  x = "Número de fracasos", 
  y = "Frecuencia") + 
  theme_minimal()+
  theme(legend.position = "none")+
  scale_fill_manual(values = c("#CD1076", "#8B3A62"))
}

geometrica_binneg(r = 8, p = 0.5, n_sim = 1000)
```
...
# 1. Simulación como suma de geométricas
# Cada geométrica cuenta defectuosos antes de cada componente bueno
n_sim_ejpro2 <- 1000000
suma_fallidos <- replicate(n_sim_ejpro2, sum(rgeom(r_tiros, prob = p_exito)))

# 2. Número esperado de intentos fallidos
intentos_fallidos <- r_tiros * (1 - p_exito) / p_exito
desv_std_fallidos <- sqrt(r_tiros * (1 - p_exito) / (p_exito^2))


# 3.Simulación para verificar
n_sim_ejpro2 <- 1000
simulacion_tiros <- replicate(n_sim_ejpro2, {
  sum(rgeom(r_tiros, prob = p_exito))
})

tiros_sim_esperados <- mean(simulacion_tiros)

cat("Ejemplo 2: Tiros baloncesto\n")
cat("-----------------\n")
cat("Probabilidad (simulación):", round(prob_simulada, 5), "\n")
cat("   Rechazos esperados:", round(intentos_fallidos, 3), "±", round(desv_std_fallidos, 3), "\n")
cat("   Simulación:", round(tiros_sim_esperados, 2), "\n\n")
```
...
#  Probabilidad entre 12 pruebas fallidas
prob_12 <- sum(dnbinom(12, size = r_pruebas, prob = p_exito))

#  Probabilidad mas de 20 pruebas fallidas
prob_mas_20 <- 1 - sum(dnbinom(12, size = r_pruebas, prob = p_exito))

# Costo esperado en pruebas fallidas
intentos_fallidos <- r_pruebas * (1 - p_exito) / p_exito
desv_std_fallidos <- sqrt(r_pruebas * (1 - p_exito) / (p_exito^2))
costo_1 <- (intentos_fallidos + desv_std_fallidos)*10000
costo_2 <- (intentos_fallidos - desv_std_fallidos)*10000


cat("EJERCICIO 3: Tiros baloncesto\n")
cat("----------------\n")
cat("Probabilidad 12 pruebas fallidas:", prob_12,"\n")
cat("   Rechazos esperados:", round(intentos_fallidos, 2), "±", round(desv_std_fallidos, 2), "\n")
cat("Costo esperado pruebas fallidas entre:", "$", costo_2, "y", "$", costo_1 ,"\n")
```
:::
