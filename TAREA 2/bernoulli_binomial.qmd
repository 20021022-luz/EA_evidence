...
::: {#def-bernoulli}
Una variable aleatoria $X$ sigue una **distribución de Bernoulli** con
parámetro $p$ si puede tomar solo dos valores: 1 (éxito) con
probabilidad $p$ y 0 (fracaso) con probabilidad $1-p$. La función de
...
A una variable aleatoria con distribución Bernoulli la denotaremos como
...
Consideremos el lanzamiento de una moneda justa, donde "Cara" se
considera un éxito (1) y "Sello" un fracaso (0). La probabilidad de
obtener cara es $p = 0.5$.
...
Consideremos variables aleatorias Bernoulli con diferentes valores de
$p$, como se muestra en la siguiente tabla:
...
parámetros $n$ y $p$ si representa el número de éxitos en $n$ ensayos
independientes de Bernoulli, cada uno con probabilidad de éxito $p$. La
función de probabilidad está dada por:
$$f(x) = \binom{n}{x} p^x (1-p)^{n-x}$$

para $x = 0, 1, 2, \ldots, n$, donde
$\binom{n}{x} = \frac{n!}{x!(n-x)!}$. A una varible aleatoria con
distribución binomial la denotaremos como:
...
Supongamos que lanzamos una moneda justa 10 veces. Queremos encontrar la
distribución del número de caras obtenidas.
...
Un proceso de manufactura produce artículos defectuosos con probabilidad
0.05. Se toman muestras de 20 artículos.
...
A continuación, exploramos cómo varía la distribución binomial al
modificar los parámetros $n$ y $p$.
...
Se realiza una simulación de 1000 experimentos de una variable aleatoria
con distribución $Binomial(n=15, p=0.4)$ y se comparan las estadísticas
observadas (media y varianza) con las teóricas, así como la distribución
observada con la teórica.
...
Si $X_1, X_2, \ldots, X_n$ son variables aleatorias independientes e
idénticamente distribuidas con $X_i \sim \text{Bernoulli}(p)$, entonces:

$$X = \sum_{i=1}^{n} X_i \sim \text{Binomial}(n,p)$$
:::

**Verificación mediante una simulación** Para verificar esta relación,
simulamos la suma de 8 variables Bernoulli independientes con $p=0$
...
Cuando $n$ es grande y $p$ no está muy cerca de 0 o 1, la distribución
binomial se puede aproximar con una normal:
...
Un nuevo medicamento tiene una probabilidad de 0.8 de curar una
enfermedad. Se prueba en 25 pacientes.
...
Una máquina produce piezas defectuosas con probabilidad 0.02. Si se
define éxito como "pieza defectuosa":
...
-   
    ...
    c)  Si se inspeccionan 10 piezas independientemente, ¿cuál es la
        probabilidad de encontrar exactamente 2 defectuosas?

```{r}
#| label: produccion-bernoulli
p_pieza <- 0.02

# Crear tabla de probabilidades
bernoulli_produccion <- data.frame(
  Resultado = c("Pieza no defectuosa (0)", "Pieza defectuosa (1)"),
  Valor = c(0, 1),
  Probabilidad = c(1-p_pieza, p_pieza)
)

kable(bernoulli_produccion, 
      caption = "Distribución Bernoulli - Producción defectuosa (p = 0.02)",
      align = "ccc") %>%
  kable_styling(bootstrap_options = "striped")

# Parámetros
media_pieza <- p_pieza
varianza_pieza <- p_pieza * (1 - p_pieza)
desv_std_pieza <- sqrt(varianza_pieza)

cat("Parámetros estadísticos:\n")
cat("Media:", media_pieza, "\n")
cat("Varianza:", varianza_pieza, "\n")
cat("Desviación estándar:", desv_std_pieza, "\n")
```
...
::: {#exr-examen}
En un examen de opción múltiple con 4 opciones por pregunta, un
estudiante responde al azar:
...
```{r}
#| label: Pregunta opciòn multiple -bernoulli
# Modelo Bernoulli
p_correcta <- 1/4
cat("Inciso b La probabilidad de acertar:", p_correcta, "\n")


#Inciso a
# Crear tabla de probabilidades
bernoulli_acertar <- data.frame(
  Resultado = c("Respuesta incorrecta (0)", "Respuesta incorrecta (1)"),
  Valor = c(0, 1),
  Probabilidad = c(1-p_correcta, p_correcta)
)

kable(bernoulli_acertar, 
      caption = "Distribución Bernoulli - respuesta correcta (p = 0.25)",
      align = "ccc") %>%
  kable_styling(bootstrap_options = "striped")

#Inciso c
# Parámetros
media_correcta <- p_correcta
varianza_correcta <- p_correcta * (1 - p_correcta)
desv_std_correcta <- sqrt(varianza_correcta)

cat("Parámetros estadísticos:\n")
cat("Media:", media_correcta, "\n")
cat("Varianza:", varianza_correcta, "\n")
cat("Desviación estándar:", desv_std_correcta, "\n")
```
...
Se lanza una moneda 15 veces: - a) ¿Cuál es la probabilidad de obtener
exactamente 8 caras? - b) ¿Cuál es la probabilidad de obtener al menos
12 caras? - c) Calcule la media y desviación estándar
...
```{r}
#| label: grafico-binomial-monedas
#| fig-cap: "Distribución Binomial - 15 lanzamientos de moneda"
ggplot(binomial_monedas, aes(x = Num_Caras, y = Probabilidad)) +
  geom_col(fill = "#B2DFEE", alpha = 0.7, width = 0.8) +
  geom_text(aes(label = round(Probabilidad, 3)), vjust = -0.5, size = 3) +
  geom_vline(xintercept = media_monedas, color = "#a2cd5a", linetype = "dashed", size = 1) +
  annotate("text", x = media_monedas + 2.5, y = max(prob_binomial) * 0.8, 
           label = paste("Media =", media_monedas), color = "#a2cd5a") +
  scale_x_continuous(breaks = k_values) +
  labs(title = paste("Distribución Binomial(", n_monedas, ",", p_monedas, ")"),
       x = "Número de Éxitos",
       y = "Probabilidad") +
...
Un medicamento tiene efectividad del 85%. Se administra a 20
pacientes: - a) ¿Cuál es la probabilidad de que se curen exactamente 18
pacientes? - b) ¿Cuál es la probabilidad de que se curen entre 15 y 19
pacientes? - c) ¿Cuántos pacientes se espera que se curen?
...
# Preguntas de interés
# 1. ¿Cuál es la probabilidad de que exactamente 18 pacientes se curen?
prob_18_curas <- dbinom(18, n_pacientes, p_cura)

# 2. ¿Cuál es la probabilidad de que al menos 22 pacientes se curen?
#prob_min_22 <- 1 - pbinom(21, n_pacientes, p_cura)

# 3. ¿Cuál es la probabilidad de que entre 15 y 19 pacientes se curen?
...
cat("Desviación estándar:", round(desv_std_curas, 4), "\n\n")
cat("Probabilidades de interés:\n")
cat("P(exactamente 18 curas):", round(prob_20_curas, 4), "\n")
#cat("P(al menos 22 curas):", round(prob_min_22, 4), "\n")
cat("P(entre 15 y 19 curas):", round(prob_18_22, 4), "\n")
...
ggplot(medicina_data, aes(x = k, y = probabilidad)) +
  geom_col(fill = "darkblue", alpha = 0.7, width = 0.8) +
  geom_vline(xintercept = media_curas, color = "red", linetype = "dashed", size = 1) +
  geom_vline(xintercept = c(15, 20), color = "orange", linetype = "dotted", size = 1) +
  annotate("text", x = media_curas + 1.5, y = max(prob_medicina) * 0.8, 
           label = paste("E[X] =", media_curas), color = "red") +
...
Sean $X$ y $Y$ dos variables aleatorias independientes con distribución
$bin(n, p)$, y $bin(m, p)$, respectivamente. Entonces la variable $X+Y$
tiene distribución $bin(n+m, p)$.
:::

::: {#Ejercicio-Binomial_5}
Solución

```{r}
#Generar datos variables independientes binomiales y sumarlas 
X <- rbinom(1000, 10, 0.5)
Y <- rbinom(1000, 10, 0.5)

Z <- X + Y 

#Comparar con la binomial directa 

 W <- rbinom(1000, 25, 0.5)
 
 datos_compara <- data.frame(
   valor = Z,
   Tipo = "Suma de binomiales"
 )
 
 datos_compara_2 <- data.frame(
   valor = W,
   Tipo = "Binomial directo"
 )
 
 datos_final <- rbind(datos_compara, datos_compara_2)
 

ggplot(datos_final, aes(x = valor, fill = Tipo)) +
  geom_histogram(position = "dodge", alpha = 0.6, center = 0, binwidth = 1) +
  scale_fill_manual(values = c("Suma de binomiales" = "purple", "Binomial directo" = "#EE7AE9")) +
  scale_x_continuous(breaks = seq(0, 25, by = 1))+
  labs(title = "Comparación: Suma de binomiales vs Binomial directo",
       x = "Valor",
       y = "Frecuencia",
       fill = "Distribución") +
  theme_minimal() +
